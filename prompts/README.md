...
두 형식을 함께 요청하면 **Core Brief → 칼럼 작성·사실 및 논리 검수 → 칼럼 요약 → 카드뉴스 검수** 순서로 만듭니다. 카드뉴스의 직접 입력은 검수한 칼럼입니다. 한 형식만 요청하면 그 형식만 만들고, 카드뉴스 단독 요청은 Core Brief를 사용합니다. 시각 자료 생성 전에 원고를 검수하고, 실제 이미지나 HTML이 만들어지면 해당 범위를 다시 검수합니다.
