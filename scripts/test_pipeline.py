...
class PipelineTests(unittest.TestCase):
    ...
    def test_html_preserves_notes_and_escapes_content(self):
        ...
        page = (result / ("html/" + renderer.card_basename(topic.name, 2) + ".html")).read_text(encoding="utf-8")
        self.assertIn("&lt;script&gt;", page)
        self.assertNotIn("<script>", page)
        self.assertIn("기능 확인용 예시입니다.", page)
        self.assertIn("가상 활용 예시", (result / ("html/" + renderer.card_basename(topic.name, 3) + ".html")).read_text(encoding="utf-8"))
        cover = (result / ("html/" + renderer.card_basename(topic.name, 1) + ".html")).read_text(encoding="utf-8")
    ...
    def test_external_output_is_portable_and_preserves_source(self):
        ...
        self.assertEqual((result / ("assets/" + renderer.card_basename(topic.name, 1) + "-example.png")).read_bytes(), (image_root / "example.png").read_bytes())
        ...
        self.assertEqual(snapshot["cards"][0]["html"], "html/2026-09-15-test-topic-card-01.html")
        for card in snapshot["cards"]:
            self.assertTrue((result / card["html"]).is_file())
            self.assertIsNone(card["png"])
        moved = self.root / "moved-delivery"
        shutil.copytree(result, moved)
        self.assertIn("data:image/png;base64,", (moved / ("html/" + renderer.card_basename(topic.name, 1) + ".html")).read_text())
